...
namespace AO_UPD_Repeat
{
    ...
    class Program
    {
        ...
        static void p9527_config()
        {
            ...
            Console.Write("Number of waveforms to be output? [0: infinite] ");
            config_para.upd_repeat_cnt = get_console_input((uint)0);
            if (config_para.upd_repeat_cnt == 0)
            {
                // Infinite number of waveforms, set iteration to 0 and definite to 0
                config_para.upd_op_definite = 0;
            }
            else
            {
                // N waveform to be output, set iteration to N and definite to 1
                config_para.upd_op_definite = 1;
            }

            // Update repeat interval
            if (config_para.upd_repeat_cnt != 1)
            {
                Console.Write("Delay interval between two output waveforms? [0] ");
                config_para.upd_repeat_interval = get_console_input((uint)0);
            }
        }
        ...
        static void Main(string[] args)
        {
            ...
            // Due to update finite number of AO samples with repeat mode, we disable it.
            ...
            do {
                ...
                // When operation is complete, DSA_AO_AsyncClear() should be called to clear no longer used settings.
                result = DSA_DASK.DSA_AO_AsyncCheck(config_para.card_handle, out config_para.op_stopped, out config_para.access_cnt);
                if (result != DSA_DASK.NoError)
                {
                    Console.Write("\nFalied to perform DSA_AO_AsyncCheck(), error: " + result);
                    exit_handle();
                }
            ...
            {
                Console.ReadLine();
                Console.Write("AO update is manually stopped!");
            }
            else
            {
