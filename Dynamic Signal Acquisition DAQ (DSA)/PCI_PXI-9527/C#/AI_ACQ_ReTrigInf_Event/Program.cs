...
namespace AI_ACQ_ReTrigInf_Event
{
    ...
    class Program
    {
        ...
        static void exit_handle()
        {
            ...
            {
                // Reset trig ready event
                DSA_DASK.DSA_AI_EventCallBack(config_para.card_handle, 0/*remove*/, DSA_DASK.TrigEvent/*EventType*/, ai_trig_ready_cbdel);
            }
        ...
        static void p9527_config()
        {
            ...
            // Re-trigger count
            config_para.retrig_count = 0; //Infinite trigger count
        ...
        // Callback function for AI trig ready event
        static void ai_trig_ready_cbfunc()
        {
            // Trigger ready with next trigger
            Console.Write("Trigger ready, trigger count: {0}\r", ++ config_para.trig_ready_cnt);
            ...
            if (config_para.file_format == 0)
            {
                ...
                DSA_DASK.DSA_AI_ContVScale(config_para.card_handle, config_para.chnl_range,
                                           config_para.raw_data_buf[config_para.trig_ready_idx], config_para.scale_data_buf,
                                           (int)config_para.buf_size);

                config_para.trig_ready_idx ++;
                config_para.trig_ready_idx %= 2;
        ...
        static CallbackDelegate ai_trig_ready_cbdel = new CallbackDelegate(ai_trig_ready_cbfunc);
        ...
        {
            Console.Write("This example performs AI acquisition with infinite re-trigger.\n");
            ...
            // Set AI trig ready event
            result = DSA_DASK.DSA_AI_EventCallBack(config_para.card_handle, 1/*add*/, DSA_DASK.TrigEvent/*EventType*/, ai_trig_ready_cbdel);
            if (result != DSA_DASK.NoError)
            {
            ...
            Console.Write("\nAI operation is started, waiting triggers from the set trigger source...\n");
