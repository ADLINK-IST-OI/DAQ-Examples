...
Module Module1
    ...
    Sub exit_handle()
        ...
        If config_para.is_done_evt_set Then
            ' Reset done event
            DSA_DASK.DSA_AI_EventCallBack(config_para.card_handle, 0, DSA_DASK.AIEnd, ai_done_cbdel)
        End If
    ...
    Sub p9529_config()
        ...
        config_para.sample_rate = get_console_input(Convert.ToDouble(tmp_sample_rate_max))
        If config_para.sample_rate < tmp_sample_rate_min OrElse config_para.sample_rate > tmp_sample_rate_max Then
            Console.Write("Warning! Invalid sample rate. Force to set to {0}." + vbCrLf, tmp_sample_rate_max)
            config_para.sample_rate = Convert.ToDouble(tmp_sample_rate_max)
        End If
    ...
    Dim ai_done_event As New AutoResetEvent(False)
    Sub ai_done_cbfunc()
        ...
        If config_para.file_format = 0 Then
            ...
            DSA_DASK.DSA_AI_ContVScale(config_para.card_handle, config_para.chnl_range, config_para.raw_data_buf_alignment, config_para.scale_data_buf, Convert.ToInt32(config_para.access_cnt))
            Console.Write("done")
        ...
        ai_done_event.Set()
    End Sub
    Dim ai_done_cbdel As New CallbackDelegate(AddressOf ai_done_cbfunc)
    ...
    Sub Main()
        ...
        config_para.raw_data_buf = Marshal.AllocHGlobal(Convert.ToInt32(Marshal.SizeOf(GetType(UInteger)) * config_para.buf_size) + 8)
        ' Adjust memory address for 16 alignment
        Dim pBufferAlign As UInt32
        If (CUInt(config_para.raw_data_buf) And &H8) Then
            pBufferAlign = CUInt(config_para.raw_data_buf) + 8
        Else
            pBufferAlign = CUInt(config_para.raw_data_buf)
        End If
        config_para.raw_data_buf_alignment = CType(pBufferAlign, IntPtr)
        config_para.scale_data_buf = New Double(config_para.buf_size - 1) {}
        config_para.buf_id_array = New UInteger(0) {}
        Dim buf_id As UShort
        result = DSA_DASK.DSA_AI_ContBufferSetup(config_para.card_handle, config_para.raw_data_buf_alignment, config_para.buf_size, buf_id)
        If result <> DSA_DASK.NoError Then
            Marshal.FreeHGlobal(config_para.raw_data_buf)
        ...
        ' Set AI done event
        result = DSA_DASK.DSA_AI_EventCallBack(config_para.card_handle, 1, DSA_DASK.AIEnd, ai_done_cbdel)
        If result <> DSA_DASK.NoError Then
            Console.Write(vbCrLf + "Falied to perform DSA_AI_EventCallBack(), error: " + result.ToString())
            exit_handle()
        End If
