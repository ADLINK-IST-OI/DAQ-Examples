#####################################################################################
# ADLINK USB-1210to acquire/measure DATA  
# This sample program shows how to import DLL
# AI One shot acquisition for USB-1210
...
dll.UD_AI_Channel_Config.restype = c_int16
dll.UD_AI_Channel_Config.argtypes = [c_uint16, c_uint16, c_uint16, c_uint16, c_uint16]
dll.UD_AI_Trigger_Config.restype = c_int16
dll.UD_AI_Trigger_Config.argtypes = [c_uint16, c_uint16, c_uint16, c_uint16, c_uint32, c_uint32, c_uint32, c_uint32]
...
cfg_card_type = np.int16(0x08) #USB-1210  
cfg_inputtype = np.int16(0x04)#UD_AI_Differential
cfg_convsrc = np.int16(0x01)#UD_AI_CONVSRC_INT=0x01, UD_AI_CONVSRC_EXT=0x02
cfg_trigsrc = np.int16(0x0380)#UD_AI_TRGSRC_SOFT
cfg_trigpolarity = np.int16(0x040)#P1902_AI_TrgPositive
cfg_trigmode = np.int16(0x000)#UD_AI_TRGMOD_POST
...
cfg_sampling_rate = np.int32(2000000) #2M 
...
var_ret = dll.UD_AI_Channel_Config(var_card \
                               ,cfg_inputtype \
                               ,cfg_inputtype \
                               ,cfg_inputtype \
                               ,cfg_inputtype \
...
if var_ret < 0:
    ...
    print("UD_AI_Channel_Config fail, error = %d\n",var_ret)
    exit()

var_ret = dll.UD_AI_Trigger_Config( var_card  \
                                  , cfg_convsrc  \
                                  , cfg_trigmode  \
                                  , cfg_trigsrc  \
                                  , 0  \
...
if var_ret < 0:
    dll.UD_Release_Card(var_card)
    print("UD_AI_Trigger_Config fail, error = %d\n",var_ret)
    exit()
...
plt.title("USB-1210")
plt.ylim(-10,10)
plt.savefig("USB-1210_python_example_result.jpg",dpi=800,format="jpg")
plt.show()
